...
function PlayerScoreCard({
  ...
  const getRankBadge = () => {
    ...
    if (rank === 2) return 'bg-gray-300 text-gray-700 dark:bg-gray-600 dark:text-gray-200';
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 cursor-pointer hover:shadow-md transition-shadow"
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">{player.playerName}</h3>
      ...
      <div className="space-y-2">
        <div>
          <div className="flex justify-between text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500 mb-1">
            <span>Skill</span>
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 grid grid-cols-5 gap-2 text-center text-xs">
        <div>
          <p className="font-semibold text-gray-900 dark:text-gray-100">{player.rawStats.matchesPlayed}</p>
          <p className="text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide">Matches</p>
        </div>
        <div>
          <p className="font-semibold text-green-600">{player.rawStats.goals}</p>
          <p className="text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide">Goals</p>
        </div>
        <div>
          <p className="font-semibold text-purple-600">{player.rawStats.assists}</p>
          <p className="text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide">Assists</p>
        </div>
        <div>
          <p className="font-semibold text-blue-600">
            {Math.round(player.rawStats.winRate * 100)}%
          </p>
          <p className="text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide">Win Rate</p>
        </div>
        <div>
          <p className="font-semibold text-yellow-600">
            {player.rawStats.averageEvaluationRating?.toFixed(1) || '-'}
          </p>
          <p className="text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide">Avg Rating</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      <header className="bg-white dark:bg-gray-800 shadow">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Performance Scores</h1>
          <button
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <div className="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center">
          <div className="inline-flex rounded-lg bg-white dark:bg-gray-800 shadow p-1">
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedGroup && (
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm font-medium text-gray-700 dark:text-gray-300"
              >
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Score Components</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                  <span className="text-gray-600 dark:text-gray-400">Skill (Evaluations)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-green-500"></div>
                  <span className="text-gray-600 dark:text-gray-400">Offense (Goals + Assists)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-cyan-500"></div>
                  <span className="text-gray-600 dark:text-gray-400">Defense (Clean Sheets)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                  <span className="text-gray-600 dark:text-gray-400">Team (Win Rate + Participation)</span>
                </div>
              </div>
            </div>
            ...
            {currentTeam.players.length === 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-8 text-center">
                <p className="text-gray-500 dark:text-gray-400">No players in this group</p>
              </div>
            )}
          </div>
        ) : null}
