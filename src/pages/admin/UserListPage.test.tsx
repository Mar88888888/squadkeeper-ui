...
describe('UserListPage', () => {
  ...
  const mockPlayers = [
    {
      ...
      weight: 55,
      strongFoot: 'RIGHT',
      group: { id: 'g1', name: 'U12', ageGroup: 'U12' },
      parent: null,
  ...
  const mockParents = [
    {
      ...
      lastName: 'One',
      phoneNumber: '123456789',
      children: [{ id: 'p2', firstName: 'Sara', lastName: 'Player' }],
  ...
  const mockGroups = [
    { id: 'g1', name: 'U12', ageGroup: 'U12' },
    { id: 'g2', name: 'U14', ageGroup: 'U14' },
  ];
  ...
  describe('Delete functionality', () => {
    it('should open delete dialog when delete button clicked', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Are you sure you want to delete "John Coach"/),
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Link child functionality', () => {
    it('should open link child modal when + button clicked', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Link Child to Parent One/),
        ).toBeInTheDocument();
      });
    });
    ...
    it('should show available players in modal', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Link Child to Parent Two/),
        ).toBeInTheDocument();
        expect(screen.getByText('Tom Player')).toBeInTheDocument();
      });
    });
  ...
  describe('Unlink child functionality', () => {
    it('should unlink child when X button clicked', async () => {
      ...
      await waitFor(() => {
        expect(mockUsersApi.unlinkChildFromParent).toHaveBeenCalledWith(
          'par1',
          'p2',
        );
      });
  ...
  describe('Empty states', () => {
    it('should show message when no coaches found', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No coaches registered')).toBeInTheDocument();
      });
    });
    ...
    it('should show message when no parents found', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No parents registered')).toBeInTheDocument();
      });
    });
    ...
    it('should show search no match message', async () => {
      ...
      expect(
        screen.getByText(
          'No coaches match your search. Try a different keyword.',
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Link child with selection', () => {
    it('should link child when player is selected', async () => {
      ...
      await waitFor(() => {
        expect(mockUsersApi.linkChildToParent).toHaveBeenCalledWith(
          'par2',
          'p1',
        );
      });
