...
describe('MyGroupsPage', () => {
  ...
  beforeEach(() => {
    ...
    mockSchedulesApi.generateTrainings.mockResolvedValue({
      created: 10,
      skipped: 2,
    });
    mockSchedulesApi.deleteFutureGenerated.mockResolvedValue({
      deleted: 5,
      kept: 2,
    });
  });
  ...
  describe('Schedule modal', () => {
    it('should open schedule modal when Training Schedule clicked', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Training Schedule - U12 Main'),
        ).toBeInTheDocument();
      });
    });
    ...
    it('should close modal when Close clicked', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText('Training Schedule - U12 Main'),
  ...
  describe('Generate trainings', () => {
    ...
    it('should show default topic input', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Default Topic (optional)'),
        ).toBeInTheDocument();
      });
    });
    ...
    it('should show Generate Trainings button', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole('button', { name: 'Generate Trainings' }),
        ).toBeInTheDocument();
      });
    });
    ...
    it('should delete future trainings when Clear Future clicked', async () => {
      ...
      await waitFor(() => {
        expect(mockSchedulesApi.deleteFutureGenerated).toHaveBeenCalledWith(
          'g1',
        );
      });
  ...
  describe('Loading state', () => {
    ...
    it('should show loading spinner while loading schedule', async () => {
      mockSchedulesApi.getSchedule.mockImplementation(
        () => new Promise(() => {}),
      );
  ...
  describe('Error handling', () => {
    ...
    it('should show error message on schedule API failure', async () => {
      mockSchedulesApi.getSchedule.mockRejectedValue(
  ...
  describe('Empty state', () => {
    it('should show message when no groups assigned', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            'You are not assigned to any groups yet. Contact an administrator if you believe this is an error.',
          ),
        ).toBeInTheDocument();
      });
    });
