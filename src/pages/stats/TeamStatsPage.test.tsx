...
describe('TeamStatsPage', () => {
  const mockTeamStats = [
    ...
    },
    {
      groupId: 'g2',
      groupName: 'U14',
      players: [createPlayerStats('p4', 'Player Four', 12, 8, 5)],
      period: 'all_time' as const,
    },
  ];
  ...
  describe('Empty state', () => {
    it('should show message when no teams found', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            'Create groups and add players to start tracking team statistics.',
          ),
        ).toBeInTheDocument();
      });
    });
    ...
    it('should show message when team has no players', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('No players in this group'),
        ).toBeInTheDocument();
      });
    });
  });
